...
OUTPUT_DIRECTORY = '/Users/brianritz/projects/findball/output/nu2_findball/'
OUTPUT_PREFIX = 'nu2_120s'
...
if __name__=="__main__":
    ...
    # take random 10%
    ind = np.arange(0, distances.shape[0])
    ...
    for ind, pitch_no, distance_ft, fps, zoom, deg, velocity in distances.itertuples():
        try:
            ...
            start = time.time()
            param1, res1 = find_speed7.find_speed(d, common_rect, distance_ft, zoom, fps, s=10.)
            end = time.time()
            elapsed = end - start
