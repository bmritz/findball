...
def find_speed(img_path, rect, distance, zoom, fps=120, s=10., constrained=True):
    ...
    if len(v2.trajectories) > 0:
        ...
        for velo_type in ['velo', 'velo_no_y', 'velo_no_d', 'velo_no_d_no_y']:
            for beg in [-1, 0, 1]:
                    for lbound in np.arange(0, 0.00005, 0.000005):
                        ((pred_velo, drag_param),reconstructed)  = ball_traj.smooth_data(velo_type=velo_type, ignore_beginning=beg, lbound=lbound)
...
#zoom=100
#fps=120
#v2= Video(d, rect=common_rect, zoom=zoom, distance=distance_ft, s=10., frame_rate=120)
...
#ball_traj.remove_outliers()
#ball_traj.find_velos(max_frame_diff=2)
#ball_traj.smooth_data(velo_type='velo', ignore_beginning=0, lbound=0.00003)
