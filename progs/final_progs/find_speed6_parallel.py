...
def smooth_vector_drag_constrained(v, x=None):
    y = pd.Series(v)
    x = pd.Series(range(len(v))) if x is None else pd.Series(x)
    ...
    constraints = ([35., 0.000015], [105., 0.000085])
...
class ContourBlob(object):
    ...
    def straight_line_velo(self, other_contour):
        """
        ...
        for i in range(num_contours):
            ...
            for j in range(i+1, min(i+max_frame_diff+1, num_contours-1)):
                ...
                if x - y <= max_frame_diff:
                    ...
                    if velo > 0:
                        all_velos.append((x, y, f1, f2, h1, h2, v1, v2, fd, d1, d2, t, velo))

        self.all_velos = pd.DataFrame(all_velos, 
                                      columns=['num1', 'num2', 'frame1', 'frame2', 
                                      'h1', 'h2', 'v1', 'v2','fd', 'd1', 'd2', 't', 'velo'])
...
#@do_cprofile
def find_speed(img_path, rect, distance, zoom, fps=120, s=10.):
    """
    ...
    if len(v2.trajectories) > 0:
        ...
        print "velos found"
        drag_params_res, smooth_velos = ball_traj.smooth_data()
        print "data smoothed"
        all_velos = ball_traj.all_velos
        
    return (drag_params_res, all_velos)
...
def fspeed(pars):
    ...
    start = time.time()
    fxn, crect, pitch_no, distance_ft, zoom, fps = pars
    ...
    x = fxn(d, crect, distance_ft, zoom, fps, s=9.)
    return x
...
if __name__=="__main__":
    ...
    p = mp.Process(target=fspeed, args=((find_speed, common_rect, 20,30, 0, 120),))
    p.start()
    p.join()
